...
Quanto à variedade, refere-se às diferentes fontes de geração dos dados, bem como aos 
diversos formatos em que esses dados se encontram, como texto, áudio, vídeo, mídias sociais 
e dados semi estruturados, como XML. Nesse contexto, "atualmente, as empresas podem coletar 
...
Diante do exposto, o Big Data configura-se como um paradigma marcado pelas características 
dos 5 Vs, superando os modelos tradicionais. Nesse contexto, a adoção de tecnologias como o 
Apache Hadoop evidencia a necessidade de novas abordagens para o tratamento dos dados. 
Assim, torna-se relevante compreender seu contexto de evolução e aplicação.
...
plataformas e soluções de Big Data \cite{khamaru2022dynamics}.
...
franchising, o uso de BI, bem como o gerenciamento de dados e de metadados. A preparação 
envolve atividades como coleta, transformação, limpeza e armazenamento. Já o data 
franchising refere-se à reorganização dos dados para fins analíticos. Por sua vez, o 
Business Intelligence possibilita o acesso e a análise desses dados por meio de 
...
das fontes e dos formatos de origem. Denominada Transform, os dados são transformados 
...
para a replicação de dados e bancos de dados. Segundo \citeonline{googlecloudetl}, as 
...
migrações. No mais, a \citeonline{ibm2026etl} reforça que, data replication (Replicação de Dados)
é frequentemente listada como um método de integração de dados que envolve ETL.

\subsection{Variações ETL e ELT}

O modelo de ETL é o mais tradicional e mais utilizado no Big Data; entretanto, não é o único. 
Existem outras variações do processo, como o ELT. Embora ETL e ELT sejam métodos de integração 
de dados, a diferença entre eles está no momento da transformação de dados. O ETL processa os 
dados transformando-os antes de carregá-los no sistema de destino. No ELT, os dados são 
carregados no sistema de destino no formato bruto e depois transformados \cite{googlecloudetl}.
...
\begin{itemize}
    ...
    \item \textbf{SSIS:} o SQL Server Integration Services (SSIS) é uma plataforma para criar integração 
    de dados em nível empresarial e soluções de transformações de dados. 
