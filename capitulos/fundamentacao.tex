...
% \subsection{Bancos de Dados Colunares e Compressão: ClickHouse}
% Pendente: falta referência bibliográfica (documentação oficial do ClickHouse e/ou
% artigo sobre bancos colunares). Conteúdo a cobrir:
%   - Bancos analíticos colunares (OLAP) vs. bancos relacionais orientados a linha (OLTP)
%   - Por que armazenamento colunar favorece compressão (dados do mesmo tipo/coluna
%     agrupados fisicamente) e consultas analíticas (leitura só das colunas necessárias)
%   - ClickHouse especificamente: velocidade de inserção em lote e economia de espaço
%     (fundamenta a redução de ~85% de armazenamento citada no Cap. 4/5)
%   - Conectar com a etapa de Load do ETL: ClickHouse como banco de destino, em
%     contraste com o data warehouse relacional já mencionado nas Seções 2.2 e 2.2.1
